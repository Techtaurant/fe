...
const POST_DETAIL_CENTER_COLUMN_CLASS_NAME = 'mx-auto w-full max-w-[728px] min-w-0 xl:col-start-2';
...
export default function PostDetail({
  ...
  const [isTableOfContentsDialogOpen, setIsTableOfContentsDialogOpen] = useState(false);
      ...
      <main className="mx-auto px-4 pt-8 pb-[calc(max(18rem,100vh)+env(safe-area-inset-bottom))] md:px-6 md:pt-12 md:pb-[calc(max(24rem,100vh)+env(safe-area-inset-bottom))]">
        {hasTableOfContents ? (
          <button
            ...
            className="border-border bg-background/95 text-foreground hover:bg-muted fixed top-20 right-4 z-[250] inline-flex h-11 w-11 items-center justify-center rounded-full border shadow-lg shadow-black/10 backdrop-blur transition-colors md:right-6 xl:hidden"
          >
        ...
        >
          ...
          {hasTableOfContents ? <PostDetailTableOfContents headings={tableOfContents} /> : null}
        </div>
        ...
        {hasTableOfContents ? (
          ...
          >
            <div className="flex items-center justify-between gap-4">
              <h2 className="text-foreground text-lg font-semibold">{t('tocTitle')}</h2>
              <button
                type="button"
